...
const [unsavedChanges, setUnsavedChanges] = useState(false);
  ...
  const handlePayment = async (e) => {
    ...
    const res = await axios.post('http://localhost:4000/user-api/payment', order);
    ...
    let options = {
      ...
      handler:async function(response){
        ...
        const validate= await axios.post('http://localhost:4000/user-api/orderValidate',body)
  ...
  const handleSaveChanges = async () => {
    ...
    if (confirmChanges) {
      try {
        ...
        const response = await axios.put('http://localhost:4000/user-api/afterChangeCartQuantity', { cartItems: localCartItems,FirstName:currentUserFirstName });
