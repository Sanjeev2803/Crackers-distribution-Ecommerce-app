...
export const UserLoginLifecycle=createAsyncThunk('user-login',async(userObj,thunkApi)=>{
    ...
    try{
        ...
        // }
        if(role==='User'){
        let res = await axios.post('https://crackers-distribution-ecommerce-pgnljzoow.vercel.app/user-api/login', userObj)
    ...
    if(role==='Seller'){
            ...
            let res = await axios.post('https://crackers-distribution-ecommerce-pgnljzoow.vercel.app/user-api/sellerlogin', userObj)
           console.log(res.data)
            //save storage in local/session storage
            if(res.data.message === "login successful"){
                localStorage.setItem('token', res.data.payload.token)
        ...
        if(role==='Admin'){
            let res = await axios.post('https://crackers-distribution-ecommerce-pgnljzoow.vercel.app/user-api/adminlogin', userObj)
            console.log(res.data)
             //save storage in local/session storage
             if(res.data.message === "login successful"){
                 localStorage.setItem('token', res.data.payload.token)
