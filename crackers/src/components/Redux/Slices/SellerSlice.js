...
export const sellerCycle = createAsyncThunk('seller-async-calls', async (formData, thunkApi) => {
    try {
        const res = await axios.post('http://localhost:4000/product-api/editproducts', formData, {
            headers: {
...
export const deleteProduct = createAsyncThunk(
    ...
    async (product, thunkAPI) => {
        try {
            const res = await axios.delete('http://localhost:4000/product-api/deleteproducts', {
