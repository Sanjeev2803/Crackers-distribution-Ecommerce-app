...
console.log(oldproductdata)
  ...
  const productEdit = async(newProduct) => {
    ...
    const formdata = new FormData();
      ...
      try {
        let res = await axios.post('http://localhost:4000/product-api/sellerEdit', formdata, {
          headers: {
      ...
      try {
        ...
        let res = await axios.post('http://localhost:4000/product-api/urlNochangeEdit', sellerspecificProduct);
        console.log(res.data);
      } catch (error) {
        console.log(error.message);
      }
